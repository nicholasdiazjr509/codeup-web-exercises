...
$.get('https://pokeapi.co/api/v2/pokemon/dragonite').done(function (info) {
    $.get(info.held_items[0].item.url).done(function(data){ //passing in the url.
        console.log(data.flavor_text_entries[6].text);//accesing properties
        //before anything, take a look at the different objects
});
//LOOK HERE
...
setTimeout(function () {
    console.log(tim);//it's outside of the request
}, 1500);//wishful thinking
//when working with ajax or get, work inside the method
End of bad practice.
*/
...
$('#submit').click(function (e) {
    e.preventDefault();
    $('#picture-example').html('<img src="image/Wedges-3s-200px.svg" alt="loading" class="kazuya">');
    var val = $('input').val().toLowerCase();
    if (val === 'dream team') {
